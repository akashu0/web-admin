...
import { JobCommissionSection } from "./JobCommissionSection";
import { JobFaqSection } from "./JobFaqSection";

/**
 * One job, for the person deciding whether to publish it.
 *
 * Read-only apart from the publish control, the FAQ and the two incentive tabs.
 * Job content is authored in the CRM, and letting web-admin edit it too would
 * mean the same opening could say different things in two places with no way to
 * tell which was current. Which FAQ it carries is not job content — it is a
 * publishing decision, so it belongs here and nowhere else. Nor is what eG earns
 * on the placement: that is commercial, and the CRM never sees it.
 *
...
export function JobViewPage() {
    ...
    // Above the early returns below — a hook after a conditional return is a
    // hook that does not run on every render.
    ...
    return (
        <div className="space-y-4">
            <BackButton onClick={() => requestLeave(() => navigate("/jobs"))} />
            ...
            {/* Controlled, and every switch routed through requestLeave.
                Radix unmounts the inactive tab's content, so an unguarded
                switch throws away a half-typed rate card exactly as navigating
                away would — the same loss, so it is asked the same way. */}
            <Tabs
                value={activeTab}
                onValueChange={(next) => requestLeave(() => setActiveTab(next))}
                className="w-full"
            >
                <TabsList className={cn("grid w-full grid-cols-3", dirty && "opacity-60")}>
                    <TabsTrigger value="details">Details</TabsTrigger>
                    {/* Named for who earns them. The tab VALUES name the
                        audience the API is asked for, which is what the stored
                        card is keyed by; the labels are what staff read. */}
                    <TabsTrigger value="agent">B2C Incentive</TabsTrigger>
                    <TabsTrigger value="parttimer">B2B Incentive</TabsTrigger>
                </TabsList>
                ...
                {/* The part-timer card — same editor, different audience. */}
                <TabsContent value="parttimer" className="mt-4">
                    <JobCommissionSection
                        vacancyId={job.vacancyId}
                        publishStatus={job.publishStatus}
                        audience="parttimer"
